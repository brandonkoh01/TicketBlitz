...
def _build_openapi_spec(base_url: str, auth_header_name: str) -> dict[str, Any]:
    spec = get_service_swagger_spec("waitlist-service")
    spec["servers"] = [{"url": base_url}]

    security_schemes = spec.get("components", {}).get("securitySchemes", {})
    internal_token = security_schemes.get("InternalToken")
    if isinstance(internal_token, dict):
        internal_token["name"] = auth_header_name

    for path, method in (("/waitlist", "get"), ("/waitlist/status/{hold_id}", "get")):
        parameters = spec.get("paths", {}).get(path, {}).get(method, {}).get("parameters", [])
        for parameter in parameters:
            if parameter.get("name") != "limit":
                continue

            schema = parameter.setdefault("schema", {})
            schema["default"] = BaseConfig.WAITLIST_DEFAULT_LIMIT
            schema["maximum"] = BaseConfig.WAITLIST_MAX_LIMIT
...
def create_app(test_config: dict[str, Any] | None = None) -> Flask:
    ...
    register_openapi_routes(
        app,
        lambda: _build_openapi_spec(
            request.host_url.rstrip("/"),
            app.config.get("INTERNAL_AUTH_HEADER", "X-Internal-Token"),
        ),
    )
