...
begin
  ...
  if not found then
    insert into users (user_id, auth_user_id, full_name, email, metadata)
  ...
  select u.user_id, v_role::app_role_t
  from users u
...
create or replace function is_current_user_organiser()
returns boolean
language sql
stable
security definer
set search_path = public
as $$
  select current_user_role() = 'organiser';
$$;
...
revoke all on function is_current_user_organiser() from public;
grant execute on function current_user_role() to authenticated;
grant execute on function is_current_user_organiser() to authenticated;
