...
const customerApiKey = (import.meta.env.VITE_CUSTOMER_API_KEY || '').trim()
...
async function request({
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new ApiClientError('Request timed out. Please try again.', { status: 408 })
    }
    ...
    throw new ApiClientError('Unable to reach TicketBlitz services. Please try again.', { payload: error })
  } finally {
    clearTimeout(timer)
  }
}
